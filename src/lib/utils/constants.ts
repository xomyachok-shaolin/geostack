...
// Настройки 3D Tiles - оптимизированы для больших моделей
export const TILESET_DEFAULTS = {
  /** Порог ошибки экранного пространства - выше = быстрее загрузка, ниже качество издалека */
  MAXIMUM_SCREEN_SPACE_ERROR: 16,  // Баланс между качеством и скоростью
  /** Кэш для тайлов - 2GB для очень больших моделей */
  CACHE_BYTES: 2 * 1024 * 1024 * 1024,
  /** Максимальное переполнение кэша - 1GB */
  ...
  /** Плотность для динамической ошибки - более агрессивная при движении */
  DYNAMIC_SCREEN_SPACE_ERROR_DENSITY: 0.00025,
  /** Фактор динамической ошибки - высокий для быстрой загрузки при движении */
  DYNAMIC_SCREEN_SPACE_ERROR_FACTOR: 8.0,
  /** Пропускать уровни детализации для быстрой загрузки */
  SKIP_LEVEL_OF_DETAIL: true,
  /** Предпочитать листовые тайлы - загружает видимые тайлы первыми */
  PREFER_LEAVES: true,
  ...
  /** Пропускать уровни */
  SKIP_LEVELS: 1,
  /** Загружать только видимые тайлы сразу */
  IMMEDIATE_LOAD: true,
...
export const LIMITS = {
  ...
  /** Максимальное количество одновременных запросов для 3D моделей (низкий чтобы не блокировать ортофото) */
  MAX_3D_MODEL_REQUESTS: 4,
} as const;
